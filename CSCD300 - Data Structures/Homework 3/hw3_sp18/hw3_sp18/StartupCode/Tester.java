...
public class Tester {
	...
	public int countSpace(String str) {	
      ...
      if(str.charAt(0) == ' ')
         return countSpace(str.substring(1)) + 1;   
      
      else   
		   return countSpace(str.substring(1)); //change this line of code as needed.
	...
	public boolean myContains(String s1, String s2){
      ...
      if(s1 == null || s2 == null || s1.isEmpty() || s2.isEmpty() || s2.length() < s1.length()) // Base Case
         return false;
	...
	public int div(int m, int n) throws IllegalArgumentException {
      ...
      if(m < n)
		...
		return div(m - n, n) + 1; //change this line of code as needed.
	}
	...
	private boolean isSum24(int arr[], int targetSum) {
      ...
      if(arr == null || arr.length == 0)
    	  return false;
      if(arr.length == 1 && arr[0] == targetSum)
    	  return true;
	...
	private void reverseArray(int a[], int low, int high) {
		...
		{
			// Swap as we traverse to the middle from both sides
			temp = a[low];
			a[low] = a[high];
			a[high] = temp;
